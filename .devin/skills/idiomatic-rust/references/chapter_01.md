...
* The struct is `small`. A common heuristic, not a numeric law, is up to 2 (maybe 3) words of memory or 24 bytes on a 64-bit target. The real cutoff depends on the target's register width and calling conventions, so measure when the path is hot.
...
> Historical source observation from the pinned handbook, not a statement about your current toolchain: as of Rust version 1.88, it was necessary to execute rustfmt in nightly to correctly reorder code `cargo +nightly fmt`. For commands requiring nightly, use your installed current rustup components and the current rustfmt documentation; do not assume this observation still holds or that support has changed since.
...
#### ✅ Coincidental duplication, left inline:
```rust
