...
Run `cargo clippy -V` in your Rust project to check that Clippy is installed. The pinned handbook recorded `clippy 0.1.86 (05f9846f89 2025-03-31)`; that is historical output, not a required version. A successful version query confirms the component is available, not that your project passes its lints. If the component is missing, run `rustup component add clippy` for the project's toolchain.
