...
2. **State the skeleton before proving.** Write the target lemma and every helper lemma with `sorry` in place of a proof, then build: `sorry` is accepted only in interactive development, so the skeleton is filled in the IDE while `isabelle build` stays the gate. Each `sorry` marks an independent work unit. Done when: every `sorry` is an identified work unit and the IDE checks the skeleton.
