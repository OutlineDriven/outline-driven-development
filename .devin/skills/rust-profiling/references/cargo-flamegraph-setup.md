...
# Custom frequency (samples/sec, higher increases precision and overhead)
cargo flamegraph --freq 997 --bin myapp   # prime frequency can reduce aliasing for periodic workloads
...
Look for:
- Wide frames near the top (hot leaves; the CPU spends time there)
...
fn configure(c: &mut Criterion) -> &mut Criterion {
    c.measurement_time(Duration::from_secs(10))  // How long to measure
     ...
     .noise_threshold(0.05)                       // Example 5% noise threshold
     .significance_level(0.05)                    // Example p-value threshold
     .confidence_level(0.95)                      // Example confidence interval width
}
```
...
// Default is WallTime. For CPU-intensive without I/O, it is usually fine.
