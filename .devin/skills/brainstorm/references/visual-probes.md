...
- Start: `node <resolved-helper-path> start --root /tmp/odin/brainstorm-visual/<run-id>`
- Start foreground: `node <resolved-helper-path> start --root /tmp/odin/brainstorm-visual/<run-id> --foreground`
- Status: `node <resolved-helper-path> status --root /tmp/odin/brainstorm-visual/<run-id>`
- Stop: `node <resolved-helper-path> stop --root /tmp/odin/brainstorm-visual/<run-id>`
...
If the helper path is unavailable or the platform cannot display a local URL cleanly, say so briefly and use the text path. Do not build a custom event system or long-lived server to compensate.
...
Never force the visual path because a local server exists.
...
Do not add click tracking, selected states, event ingestion, forms, analytics, or "submit" affordances. Do not ask the user to click an option. Ask them to look at the artifact and reply in chat with the choice, mix, or correction.

If no interactive question tool is available, use this plain-chat fallback after showing the artifact:

> I am showing three rough options. Reply here with A, B, C, or "mix", plus anything that feels off. Judge the behavior shape, not the exact styling.
...
```text
/tmp/odin/brainstorm-visual/<run-id>/
  screens/
    001-<decision>.html
  state/
    display-info.json
...
Use `.context/odin/brainstorm-visual/<run-id>/` only when the user explicitly wants to inspect, preserve, or curate the sketches after the session. The final requirements doc in `docs/plans/` is the durable artifact.
