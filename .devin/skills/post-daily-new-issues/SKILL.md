...
description: 'Use when a human explicitly requests the daily on-call issue digest from a named issue tracker for a configured Slack channel. Not for posting without explicit human authorization for that run.'
...
| Authority | Remote: posts one Slack message in the configured channel; requires explicit human invocation. Preview the bounded issue-tracker query, destination channel, release URL, and posting consequence before using credentials. |
| Side effect | Read only the stated UTC window from the stated issue-tracker scope and, only when it contains issues, create one Slack message in the configured channel containing the total and at most three issue bullets. |
...
Require the UTC window start and end, the name of the issue tracker the digest reads from, its organization and project scope, its read credentials, the destination Slack channel, Slack posting credentials, and the release URL. The end must be later than the start, both bounds must include an explicit UTC offset, and the issue-tracker and Slack credentials must be limited to the stated scopes. An optional human-supplied label may identify the digest; it must not alter the query window or destination. Do not infer a missing bound, scope, channel, credential, or release URL.

## Procedure

1. Before accessing either service, present the UTC window, the issue tracker's organization and project, Slack channel, release URL, and the consequence that a nonempty result creates one remote Slack message. Obtain explicit human authorization for this exact run; if it is absent, stop without using credentials. Done when: explicit human authorization is obtained for this exact run.
...
3. Fetch issues created in the inclusive-start, exclusive-end UTC window from only the authorized issue-tracker organization and project. Preserve each issue's count and display identity needed for the digest; do not mutate the issue tracker. Done when: issue-tracker issues are fetched from the authorized scope within the stated window.
