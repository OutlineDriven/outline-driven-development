...
- `severity`: one of `"P0"`, `"P1"`, `"P2"`, `"P3"` — use these exact strings because the synthesis pipeline pattern-matches on them. Do not use `"high"`, `"medium"`, `"low"`, `"critical"`, or any other vocabulary, even if your persona's prose discusses priorities in those terms conceptually.
...
- Every finding must include at least one evidence item (a direct quote from the document) because the walk-through and bulk-preview render it verbatim for the user.
...
- **Always produce substantive content.** `why_it_matters` is required by the schema because every downstream surface depends on it. Empty strings, nulls, and single-phrase entries are validation failures. If you found something worth flagging at anchor `50` or higher, you can explain it: the field exists because every finding needs a reason.
...
False-positive categories to actively suppress. Do not emit a finding when any of these apply, not even at anchor `25` or `50`. These are not edge cases you should route to FYI; they are non-findings, and surfacing them dilutes the actionable set.
...
**Precedence over the false-positive catalog.** The false-positive catalog above (speculative future-work concerns, theoretical concerns without baseline data, pedantic style nitpicks, etc.) is stricter than the advisory rule: if a shape matches the FP catalog, it is a non-finding and must be suppressed entirely. Do not route it to anchor `50` / FYI because the advisory rule applies only to shapes that are not in the FP catalog.
</output-contract>
