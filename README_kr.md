...
### 운영 계약 (Operational Contract)

| 인터페이스 | 입력 | 출력 | 불변량 (Invariants) | 오류 모드 |
| --- | --- | --- | --- | --- |
| 아웃라인 컴파일러 | 인간의 의도, 규제 제약, 인수 테스트 | 표준 아웃라인 그래프 | 아웃라인 해시는 단조적임; 스키마 버전은 엄격하게 증가 | 스키마 드리프트 또는 누락된 불변량 발생 시 거부 |
| 실행 오케스트레이터 | 아웃라인 슬라이스, 툴체인 핸들 | 빌드/테스트 아티팩트, 원격 측정 패킷 | 아웃라인 노드 ID가 엔드-투-엔드로 보존됨; 도구 호출은 멱등성(idempotent)을 가짐 | 원격 측정이 누락되거나 diff 노이즈 > 임계값일 때 닫힌 상태로 실패(Fail closed) |
| 거버넌스 루프 | 원격 측정, 인간 검토, 감사 | 업데이트된 아웃라인 델타, 위험 보고서 | 배포 전 승인 기록; 감사 로그는 추가만 가능(append-only) | 승인이 지연되거나 감사 추적이 손상된 경우 인시던트 제기 |

### 엣지 케이스 및 적대적 입력 (Edge Cases & Adversarial Inputs)

1. **Tool drift (도구 드리프트):** 배터리(예: `ast-grep`)가 누락되거나 구식이면, 스텁(stub) 모드로 폴백하고 도구 동등성이 복원될 때까지 승격을 차단합니다.
2. **Ambiguous intent (모호한 의도):** 아웃라인 컴파일러는 LLM 실행을 허용하기 전에 최소한의 구체성(행위자, 데이터 경계, 예산)을 강제합니다.
3. **Spec regression (명세 회귀):** 불변량을 제거하는 모든 아웃라인 델타는 수동 검토 및 카오스 테스트 재생을 트리거합니다.
4. **Long tail concurrency (롱테일 동시성):** 비동기 도구가 예산을 초과하면, 오케스트레이터는 결정론을 보존하기 위해 직렬화된 모드로 에스컬레이션합니다.

### 검증 계획 (Verification Plan)

- **Unit (단위):** 노드별 아웃라인 스키마 린트 + AST 검증.
- **Integration (통합):** 아웃라인을 리하이드레이션(rehydrate)하고 파일 시스템/출력 다이제스트를 비교하는 결정론적 재생 하니스.
- **Property (속성):** LLM 준비 프롬프트와 아웃라인 명세 간의 퍼즈(fuzz) 차이를 통해 금지된 기능이 유출되지 않음을 증명.
- **Resilience (탄력성):** 실행 중간에 도구를 강제로 종료하여 오케스트레이터의 재시도가 아웃라인 순서를 준수하는지 확인하는 카오스 훅.

### 추적성 매트릭스 (Traceability Matrix)

| 다이어그램 | 목표 | 관련 불변량 |
| --- | --- | --- |
| 아키텍처 Δ | 아웃라인-툴체인 인터페이스 매핑 | 아웃라인 해시 단조성 |
| 데이터 흐름 Δ | 데이터가 검증을 우회하지 않도록 보장 | 계약에 구속된 IO만 허용 |
| 동시성 Δ | 발생 전(happens-before) 관계 보존 | 순환 대기 없음; 데드락 자유 |
| 메모리 Δ | 소유권 + 지속성 모델 보장 | 누수 없는 캐시, 추가 전용(append-only) 감사 |
| 최적화 Δ | 예산을 피드백 루프에 연결 | 회귀 발생 시 결정론적 롤백 트리거 |
